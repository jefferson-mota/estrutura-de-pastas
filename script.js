document.addEventListener('DOMContentLoaded', () => {
    ...
    // Liga o motor do banco de dados
    ...
    // ==========================================
    // --- CRIADOR DE MODAIS CUSTOMIZADOS ---
    // ==========================================
 ...
 // ==========================================
    // --- MÁQUINA DO TEMPO E BOTÃO LIMPAR ---
    // ==========================================
    let historyStack = [];
    let historyIndex = -1;
    let isHistoryAction = false; 
    window.isRenamingActive = false; // <-- A CHAVE GERAL DO PAUSE (Anti-Fantasma Definitivo)

    const btnUndo = document.getElementById('btn-undo');
    const btnRedo = document.getElementById('btn-redo');
    const btnClear = document.getElementById('btn-clear');

    // 1. O Fotógrafo (Blindado contra a caixinha de renomear)
    window.saveHistoryState = function() {
        if (typeof editor === 'undefined' || !editor) return;
        if (window.isRenamingActive) return; 

        // ANTI-FANTASMA VISUAL: Se a caixinha de input estiver na tela, a foto é proibida!
    ...
    // 2. Desfazer (Com salto temporal sobre o fantasma)
    window.performUndo = function() {
        if (historyIndex > 0) {
            historyIndex--;
            
            // O EXORCISTA: Se a foto for do "Novo_Item", pula para a anterior imediatamente!
    ...
    // 3. Refazer (Com salto temporal pro futuro)
    window.performRedo = function() {
        if (historyIndex < historyStack.length - 1) {
            historyIndex++;
            
            // O EXORCISTA: Pula o fantasma se ele estiver no caminho do refazer!
    ...
    // 4. Ligar os Botões da Tela (Uma vez só e com preventDefault para não bugar a tela)
    if (btnUndo) {
    ...
    // 5. Botão Limpar (O seu código mantido perfeitamente)
    if (btnClear) {
        btnClear.addEventListener('click', async () => {
            if (editor.value.trim() !== '') {
                ...
                if (res) {
                    ...
                    updateTree();
                    window.saveHistoryState(); // Tira uma foto da tela em branco para você poder dar CTRL+Z se arrepender!
                }
            }
        });
    }
    ...
    // ==========================================
    // --- RENDERIZAÇÃO E PARSER DA ÁRVORE ---
    // ==========================================
    function updateTree() {
        ...
        applySearch();
        if (typeof lucide !== 'undefined') lucide.createIcons();
        setUnsavedState(true); updateMenuStates();
    }
...
// --- INTELIGÊNCIA DE ÍCONES E CORES (NOVO) ---
    function getFileInfo(filename) {
        // Pega tudo que vem depois do último ponto (ex: "logo.svg" -> "svg")
        ...
        switch (ext) {
            ...
            case 'jpg': case 'jpeg': case 'png': case 'gif': case 'svg': case 'webp': case 'ico': 
                return { icon: 'image', color: 'file-color-img' };
            // Documentos
            ...
            case 'zip': case 'rar': case '7z': case 'tar': case 'gz': 
                return { icon: 'file-archive', color: 'file-color-archive' };
            // Multimídia
            ...
            // Configurações / Escondidos
            case 'env': case 'gitignore': case 'config': 
                return { icon: 'settings', color: 'file-color-config' };
            // Padrão (Markdown, txt, arquivos sem formato reconhecido)
            case 'md': case 'txt': 
                return { icon: 'file-text', color: 'file-color-default' };
            default: 
                return { icon: 'file', color: 'file-color-default' };
        }
    }
    ...
    function renderTreeHTML(nodes) {
        ...
        nodes.forEach(node => {
            if (node.isFile) {
                // Chama a inteligência artificial de ícones aqui!
 ...
 // ==========================================
    // --- RENOMEAR INLINE (COM PAUSE DO HISTÓRICO) ---
    // ==========================================
    function startRenaming(liElement, isNewItem = false) {
        // 1. LIGA O PAUSE DO HISTÓRICO IMEDIATAMENTE!
        ...
        // Cria a caixinha de texto na tela
        ...
        // 2. A FUNÇÃO QUE FINALIZA E SALVA
        const finishEdit = (isCancelled = false) => {
            ...
            // DESLIGA O PAUSE ANTES DE DESENHAR A ÁRVORE E BATER A FOTO!
            window.isRenamingActive = false; 

            updateTree();
            
            // Só bate a foto se a pessoa não cancelou (Esc)
            if (!isCancelled) {
                if (typeof setUnsavedState === 'function') setUnsavedState(true);
                if (typeof updateMenuStates === 'function') updateMenuStates();
        ...
        // 3. OS ESCUTADORES DA CAIXINHA (Enter, Esc e Clique Fora)
        ...
        input.addEventListener('keydown', (e) => {
            ...
            e.stopPropagation(); // Impede que o Enter vaze pro editor preto
        });
    ...
    // Variável para controlar o tempo de pausa da digitação (escutador)
    let debounceHistoryTimeout;

    // O Escutador Universal do Editor Turbinado
    editor.addEventListener('input', () => {
        // 1. Ações imediatas (acontecem a cada letra digitada)
        updateTree();
        syncVisualScroll(); // <-- A sincronização de rolagem mantida aqui!
        
        // 2. Acende a bolinha vermelha e atualiza os menus
        if (typeof setUnsavedState === 'function') setUnsavedState(true);
        if (typeof updateMenuStates === 'function') updateMenuStates();
        
        // 3. MÁGICA DO DEBOUNCE: Só tira a "foto" para o CTRL+Z se parar de digitar por 1000ms
    ...
    // 4. MÁGICA DO ESPAÇO E ENTER (Integrado ao seu keyup)
    editor.addEventListener('keyup', (e) => {
        syncVisualScroll(); // Mantém a sua sincronização visual
        
        // Se apertou Espaço ou Enter, a palavra/linha acabou. Salva a foto na hora!
        if (e.key === ' ' || e.key === 'Enter') {
            clearTimeout(debounceHistoryTimeout); // Cancela o timer para não duplicar
            if (typeof saveHistoryState === 'function' && typeof isHistoryAction !== 'undefined' && !isHistoryAction) {
                saveHistoryState();
            }
        }
    ...
    // ==========================================
    // EVENTOS DE TECLADO DO EDITOR (A MORDAÇA DIRETA)
    // ==========================================
    editor.addEventListener('keydown', function(e) {
        // 1. O SEU TAB PERFEITO
        if (e.key === 'Tab') {
            e.preventDefault();
        ...
        // 2. BLOQUEIO ABSOLUTO DO DESFAZER/REFAZER NATIVO NO TEXTAREA!
        if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'z') {
            e.preventDefault(); // Para o navegador AGORA!
            e.stopPropagation(); // Não deixa a tecla subir pro resto do código
            
            if (e.shiftKey) {
                if (typeof window.performRedo === 'function') window.performRedo();
                else if (typeof performRedo === 'function') performRedo();
            } else {
                if (typeof window.performUndo === 'function') window.performUndo();
                else if (typeof performUndo === 'function') performUndo();
            }
        }
    ...
    // ==========================================
    // EVENTOS GERAIS DA PÁGINA (ATALHOS GLOBAIS)
    // ==========================================
    document.addEventListener('keydown', (e) => {
        ...
        if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 's') {
            e.preventDefault(); 
            if (typeof window.salvarProjetoNaNuvem === 'function') {
                window.salvarProjetoNaNuvem();
            } else if (typeof btnSaveCloud !== 'undefined' && btnSaveCloud) {
                btnSaveCloud.click(); 
            }
        }
    });
    ...
    treeContainer.addEventListener('drop', (e) => {
        ...
        clearSelection();
        setUnsavedState(true); // <-- ALARME LIGADO APÓS ARRASTAR 
    });
    ...
    // ==========================================
    // --- EVENTOS DE CLIQUE NA ÁRVORE (MODAIS) ---
    // ==========================================
    treeContainer.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('.add-root-btn')) {
            ...
            setUnsavedState(true); // <-- COLE AQUI
        ...
        if (e.target.closest('.tree-delete-btn')) {
            ...
            const res = await showCustomConfirm(
                "Excluir Pasta",
                "Você está excluindo uma pasta. O que deseja fazer com os arquivos e subpastas que estão dentro dela?",
                [
        ...
        // Lógica para Abrir e Fechar as Pastas ao clicar
    ...
    // ==========================================
    // --- MENUS, TEMA, SIDEBAR E EXPORTAÇÃO ---
    // ==========================================
    ...
    // --- MENU MOBILE (HAMBURGER E SIDEBAR) ---
    if (hamburgerBtn && toolbarLinks) {
        hamburgerBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            toolbarLinks.classList.toggle('show');
            hamburgerBtn.classList.toggle('is-active'); // Agora isso apenas aciona o CSS maravilhoso!
        });
    }
    ...
    // Clicar fora para fechar dropdowns e sidebar mobile
    document.addEventListener('click', (e) => {
        if (hamburgerBtn && toolbarLinks && !e.target.closest('.toolbar-links') && !e.target.closest('#mobile-menu-btn')) {
            ...
            hamburgerBtn.classList.remove('is-active'); // Volta a animação para o sanduíche
        }
    ...
    document.addEventListener('click', (e) => {
        if (hamburgerBtn && toolbarLinks && !e.target.closest('.toolbar-links') && !e.target.closest('#mobile-menu-btn')) {
            if (toolbarLinks.classList.contains('show')) {
                ...
                // Volta o ícone para o sanduíche
    ...
    if (btnImport && fileImport) {
        btnImport.addEventListener('click', async () => {
            if (editor.value.trim() !== '') {
                const res = await showCustomConfirm(
                    "Importar Arquivo",
                    "Importar um arquivo apagará a sua estrutura atual na tela. Deseja continuar?",
                    [
                        { label: "Cancelar", value: false, class: "btn-outline" },
        ...
        fileImport.addEventListener('change', (e) => {
            ...
            reader.onload = (event) => {
                ...
                // Esconde a sidebar no mobile após importar
                if (toolbarLinks) {
                    toolbarLinks.classList.remove('show');
    ...
    const exportOptions = [
        { label: '<i data-lucide="image" class="icon-sm"></i> Exportar Imagem (.PNG)', action: exportToPng },
        { label: '<i data-lucide="folder-archive" class="icon-sm"></i> Exportar Pastas (.ZIP)', action: exportToZip },
        { label: '<i data-lucide="file-json" class="icon-sm"></i> Exportar Estrutura (.JSON)', action: exportToJson },
        { label: '<i data-lucide="file-text" class="icon-sm"></i> Exportar Texto (.TXT)', action: exportToTxt }
    ];
    ...
    if (btnExport) {
        const wrapperExport = btnExport.closest('.dropdown-wrapper');
        if (wrapperExport) wrapperExport.appendChild(exportMenu);

        btnExport.addEventListener('click', (e) => {
            e.stopPropagation();
            const isShowing = exportMenu.classList.contains('show');
            closeAllDropdowns(); 
            if (!isShowing) exportMenu.classList.add('show');
        });
    }

    // ==========================================
    // --- FUNÇÕES FINAIS DE DOWNLOAD (ARQUIVOS) ---
    // ==========================================
    async function saveFileWithDialog(blob, defaultFilename, acceptTypes, description) {
    ...
    async function exportToTxt() {
        if (editor.value.trim() === '') return alert("A estrutura está vazia!");
        const blob = new Blob([editor.value], { type: "text/plain;charset=utf-8" });
        await saveFileWithDialog(blob, "estrutura_simulacao.txt", { 'text/plain': ['.txt'] }, "Arquivo de Texto");
    }
    ...
    async function exportToJson() {
        ...
        await saveFileWithDialog(blob, "estrutura_simulacao.json", { 'application/json': ['.json'] }, "Arquivo JSON");
    }

    async function exportToZip() {
        if (editor.value.trim() === '') return alert("A estrutura está vazia!");
        if (typeof JSZip === 'undefined') return alert("Erro: A biblioteca JSZip não foi carregada. Verifique seu index.html.");
        ...
        try {
            ...
            await saveFileWithDialog(blob, "Projeto_Estrutura.zip", { 'application/zip': ['.zip'] }, "Arquivo Compactado ZIP");
        } catch (error) {
    ...
    async function exportToPng() {
        if (editor.value.trim() === '') return alert("A estrutura está vazia!");
        if (typeof html2canvas === 'undefined') return alert("Erro: Biblioteca html2canvas não foi carregada. Verifique o index.html.");
        ...
        try {
            ...
            canvas.toBlob(async (blob) => {
                await saveFileWithDialog(blob, "Estrutura_Visual.png", { 'image/png': ['.png'] }, "Imagem PNG");
            });
        } catch (error) {
    ...
    if (btnMindmap) {
        btnMindmap.addEventListener('click', () => {
            ...
            // Esconde a sidebar no mobile ao abrir o Modal
            if (toolbarLinks) {
                toolbarLinks.classList.remove('show');
                if (hamburgerBtn) hamburgerBtn.classList.remove('is-active');
    ...
    if (btnExportMindmap) {
        btnExportMindmap.addEventListener('click', async () => {
            ...
            if (!svgElement) return alert("O fluxograma ainda não foi gerado na tela.");
            ...
            try {
                ...
                canvas.toBlob(async (blob) => {
                    await saveFileWithDialog(blob, "Fluxograma_Projeto.png", { 'image/png': ['.png'] }, "Imagem PNG");
                });
            } catch (error) {
                mindmapContainer.style.overflow = originalOverflow;
                console.error("Erro ao gerar PNG do fluxograma:", error);
                alert("Ocorreu um erro ao exportar a imagem do Fluxograma.");
            }
        });
    }
    ...
    if (btnExportSvg) {
        btnExportSvg.addEventListener('click', async () => {
            ...
            try {
                await saveFileWithDialog(blob, "Fluxograma_Vetor.svg", { 'image/svg+xml': ['.svg'] }, "Vetor SVG");
            } catch (error) {
                console.error("Erro ao gerar SVG:", error);
                alert("Ocorreu um erro ao exportar o vetor SVG.");
            }
        });
    }
...
// ==========================================
    ...
    let currentUser = null; // Guarda quem é o usuário logado
    ...
    // 2. Abrir Modal ou Fazer Logout
    if (btnLoginModal) {
        btnLoginModal.addEventListener('click', async () => {
            if (currentUser) {
                // Se já estiver logado, pergunta se quer sair
                const res = await showCustomConfirm(
                    "Sair da Conta", 
                ...
                modalAuth.classList.add('show'); // Mostra a tela de login
            }
        });
    }

    if (closeAuth) {
        closeAuth.addEventListener('click', () => modalAuth.classList.remove('show'));
    }

    // 3. Ação de Submeter (Login ou Cadastro real no Supabase)
    ...
    // 4. O Grande "Escutador" (Atualiza a interface automaticamente)
    ...
    if (window.supabase) {
        supabase.auth.onAuthStateChange((event, session) => {
            if (session) {
                ...
                if (btnMyProjects) btnMyProjects.style.display = 'inline-flex'; // Mostra Meus Projetos
            } else {
                // Usuário DESLOGADO
                currentUser = null;
                ...
                if (btnMyProjects) btnMyProjects.style.display = 'none'; // Esconde Meus Projetos
            }
            if (typeof lucide !== 'undefined') lucide.createIcons();
        });
    }
...
// ==========================================
    // --- O CÉREBRO: WORKSPACE, NUVEM E MENU ---
    // ==========================================
    ...
    // --- 1. FUNÇÕES DE STATUS E PROTEÇÃO ---
    function updateProjectUI(name) {
        ...
        if (nameTextSpan) {
            nameTextSpan.textContent = currentProjectName;
        }
    }
    ...
    // --- 2. FUNÇÃO GLOBAL DE SALVAR (Usada pelo Menu e CTRL+S) ---
    ...
    // --- 3. ATALHOS DE TECLADO SEGUROS (Salvar, Desfazer, Refazer) ---
    document.addEventListener('keydown', (e) => {
        // Salvar na Nuvem (CTRL+S)
        if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 's') {
            e.preventDefault(); 
        ...
        // Desfazer - CTRL+Z (Bloqueia o nativo e usa a nossa máquina do tempo agrupada)
        if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'z' && !e.shiftKey) {
            e.preventDefault(); // Impede de apagar letra por letra
    ...
    // --- 4. COMPORTAMENTO DO MENU ARQUIVO ---
    if (btnArquivo) {
        btnArquivo.addEventListener('click', (e) => {
            e.stopPropagation();
            const isOpen = menuArquivoDropdown.classList.contains('show');
            closeAllDropdowns(); // Fecha tudo que estiver aberto
            if (!isOpen) { // Só abre se já não estivesse aberto!
    ...
    const btnExportNested = document.getElementById('btn-export-nested');
    if (btnExportNested) {
        btnExportNested.addEventListener('click', (e) => {
            if (window.innerWidth <= 768) {
                e.stopPropagation();
                btnExportNested.parentElement.classList.toggle('open');
            }
        });
    }

    // --- 5. EVENTOS DOS BOTÕES DO MENU ---
    document.getElementById('menu-novo').addEventListener('click', async () => {
        menuArquivoDropdown.classList.remove('show');
        if (hasUnsavedChanges) {
    ...
    document.getElementById('menu-meus-projetos').addEventListener('click', async () => {
        ...
        try {
            ...
            data.forEach((proj) => {
                ...
                // Cria uma linha (container) para o projeto + botão excluir
                const itemDiv = document.createElement('div');
                itemDiv.style.cssText = 'display: flex; gap: 8px; margin-bottom: 8px;';

                // Botão principal (abre o projeto)
                const btn = document.createElement('button');
                ...
                btnDel.addEventListener('click', async (e) => {
                    ...
                    // O Alerta Triplo que você pediu
                    ...
                    // Começa o processo de exclusão
                    try {
                        // Se o usuário pediu backup, nós buscamos o texto e FORÇAMOS a janela "Salvar Como"
                        if (action === "backup") {
                            ...
                            try {
                                // Tenta usar a API moderna (Abre a janela de escolher pasta no PC)
                                if (window.showSaveFilePicker) {
                                    ...
                                    // Navegador antigo ou Celular (baixa direto)
                                    throw new Error("API não suportada");
                                }
                            } catch (err) {
                                // Se o erro for 'AbortError', significa que o usuário clicou em "Cancelar" na janela do Windows/Mac
                                if (err.name === 'AbortError') {
                                    showToast("❌ Exclusão cancelada (Você não salvou o backup).");
                                    return; // PARA A EXCLUSÃO AQUI! Não apaga da nuvem.
                                } else {
                                    // Fallback para baixar direto se o navegador não suportar a janela
                                    const blob = new Blob([conteudoTXT], { type: 'text/plain;charset=utf-8' });
                                    const link = document.createElement('a');
                                    link.href = URL.createObjectURL(blob);
                                    link.download = nomeArquivo;
                                    link.click();
                                    showToast("✅ Backup baixado na pasta Downloads!");
                                }
                            }
                        }
                        ...
                        // Remove a linha visualmente do Modal
                        itemDiv.remove();
                        showToast("🗑️ Projeto excluído da nuvem!");

                        // Se você apagou o projeto que está usando AGORA na tela, a tela limpa.
    ...
    document.getElementById('menu-exp-png').addEventListener('click', () => { menuArquivoDropdown.classList.remove('show'); exportToPng(); });
    document.getElementById('menu-exp-zip').addEventListener('click', () => { menuArquivoDropdown.classList.remove('show'); exportToZip(); });
    document.getElementById('menu-exp-json').addEventListener('click', () => { menuArquivoDropdown.classList.remove('show'); exportToJson(); });
    document.getElementById('menu-exp-txt').addEventListener('click', () => { menuArquivoDropdown.classList.remove('show'); exportToTxt(); });
    ...
    // --- 6. CARREGAR PROJETO AO ABRIR O SITE ---
    ...
    carregarProjetoDaNuvem().then(() => {
        ...
        // TIRA A FOTO INICIAL PARA O CTRL+Z SABER O PONTO DE PARTIDA!
        saveHistoryState();
    });
});